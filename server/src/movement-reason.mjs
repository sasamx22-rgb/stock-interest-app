...
export function summarizeMovementReason(quote, news = []) {
  const usable = news
  ...
  if (usable.length === 0) {
    return {
      category: 'unknown',
      label: '원인 확인 중',
      summary: `현재 수집된 뉴스만으로는 ${quote.name}의 ${directionLabel(quote.changePercent)} 원인을 특정하기 어렵습니다.`,
      confidence: 'low',
      evidence: [],
    };
  }
  ...
  const representative = usable.find((item) => item.category?.id === dominant?.id) ?? usable[0];
  const categoryLabel = dominant?.count ? dominant.label : '주요 뉴스';
  const volumeText = quote.volumeRatio >= 2
    ? ` 거래량도 최근 기준의 약 ${quote.volumeRatio.toFixed(1)}배로 늘었습니다.`
    : '';

  return {
    category: dominant?.count ? dominant.id : 'news',
    label: categoryLabel,
    summary:
      ...
      + volumeText,
    confidence: dominant?.count >= 2 ? 'medium' : 'low',
    evidence: usable.slice(0, 3).map((item) => ({
      type: 'news',
