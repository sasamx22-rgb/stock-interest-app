...
개인용 Android 주식 관심도 앱입니다. 한국·미국 관심 종목, 오전·프리마켓 보고서, 뉴스와 경제 일정을 한곳에서 확인합니다.
...
- **첫 출시에서는 급등 탐지·원격 푸시를 기본 비활성화합니다.** 전체시장 커버리지와 거래량 배수 계약이 충분히 검증된 뒤 후속 기능으로 다시 켭니다.

## 현재 구현 범위

- 보고서 종목·내 관심종목을 합친 **오늘의 관심 종목**
- 매일 자동 선정되는 **오늘 꼭 볼 3종목**
- 한국·미국 관심 종목 대시보드
- 앱에서 네이버 종목 검색 후 관심종목 추가·삭제
- 관심종목 서버 로컬 JSON 영구 저장
- 앱 내부 종목 상세: 현재가·최근 일별 시세·최근 뉴스·상승·하락 원인 요약·네이버 원문 링크
- 선택형 GPT-5.6 Terra 보강: 종목 변동 원인만 보강(뉴스·일정·가격 변화 기반)
...
첫 출시에서는 급등 탐지와 원격 푸시를 사용하지 않습니다.

- 서버 기본값: `SURGE_ALERTS_ENABLED=false`
- 모바일 기본값: `EXPO_PUBLIC_SURGE_ALERTS_ENABLED=false`
- 급등 탭과 알림 기준 설정은 첫 버전에서 숨깁니다.
- 서버는 ranking 후보 조회와 2분 polling을 시작하지 않습니다.
- 모바일은 Expo push token을 등록하지 않습니다.
- 전체시장 커버리지와 거래량 배수의 단위·분모·세션을 검증한 뒤 후속 버전에서 다시 활성화합니다.

관련 코드는 후속 개발을 위해 저장소에 유지하지만, 첫 버전의 제품 기능으로 간주하지 않습니다.

## 개인화 홈 기능

### 오늘의 관심 종목

홈에서는 아래 두 경로를 하나로 합쳐 중복 제거한 목록을 보여줍니다.

- 내 관심종목
- 오늘 08:00 / 08:50 보고서에서 언급된 종목

같은 종목이 두 경로에 모두 포함되면 `내 관심`과 `보고서` 배지를 함께 표시합니다.

### 오늘 꼭 볼 3종목

통합 관심 종목에서 매일 3개를 자동 선정합니다. **내 관심 + 보고서 + 가격 움직임 + 최근 주요 뉴스 + 기업 경제일정(실적·배당 등)**을 점수화하며, 여러 신호가 겹치는 종목을 우선합니다. 검증되지 않은 거래량 배수는 첫 버전 점수에 사용하지 않습니다. 이 선정 로직 자체는 GPT를 호출하지 않아 홈을 열 때 AI 비용이 발생하지 않습니다. 최근 뉴스는 5분 캐시해 반복 요청을 줄입니다.
...
OpenAI API 키가 없어도 앱의 시세, 일정, 관심종목, 보고서와 규칙 기반 변동 원인 분석은 정상 동작합니다.
...
첫 출시에서는 급등 알림 기능이 비활성화되어 앱 설정 화면에 상승률·거래량 기준을 노출하지 않습니다. 후속 버전에서 기능을 다시 활성화할 때 기존 서버 규칙 저장 구조를 재사용할 수 있습니다.

## 종목 상세

홈이나 **관심** 탭의 종목을 누르면 앱 내부 상세 화면으로 이동합니다.
...
- PDF는 **업로드 또는 마지막 교체 시점부터 30일간 보관**한 뒤 자동 삭제합니다.
- 만료 정리는 서버 시작 시 한 번 실행되고, 실행 중에는 6시간마다 반복됩니다. PDF가 만료되어도 보고서 요약/본문은 유지되고 `pdfUrl`만 제거됩니다.
...
급등 후보용 네이버 ranking 어댑터 코드는 후속 기능을 위해 유지하지만, 첫 출시에서는 `SURGE_ALERTS_ENABLED=false`가 기본이므로 ranking 조회를 수행하지 않습니다. 전체시장 커버리지와 거래량 배수 계약이 검증된 뒤 활성화 여부를 다시 결정합니다.
...
보고서 저장, 관심종목, 시세 조회와 일정 API를 위해 서버는 계속 실행합니다. 첫 출시에서는 급등 원격 푸시는 비활성입니다. 저장소 루트의 `Dockerfile`은 서버만 포함하는 가벼운 컨테이너이며 기본 `DATA_DIR=/data`를 사용합니다.
...
- `SURGE_ALERTS_ENABLED`: 첫 출시 기본 `false`
- `PUSH_INTERVAL_SECONDS`: 급등 기능을 나중에 활성화할 경우 감시 주기. 기본 120
...
첫 출시에서는 급등 알림을 사용하지 않으므로 FCM/Expo Push 설정은 APK 베타 시작의 필수 조건이 아닙니다. 향후 급등 기능을 다시 활성화할 때 EAS 프로젝트와 FCM v1 자격증명을 연결해 실기기 검증합니다.
...
- `SURGE_ALERTS_ENABLED`: 급등 탐지/푸시 기능 플래그. 첫 출시 기본 `false`
- `NAVER_KR_MOVERS_URL`: 후속 급등 기능에서 국내 후보 URL을 바꿀 때 지정
- `NAVER_US_MOVERS_URL`: 후속 급등 기능에서 미국 후보 URL을 바꿀 때 지정
- `PUSH_INTERVAL_SECONDS`: 급등 기능을 활성화했을 때 감시 주기. 기본 120초, 최소 60초
